...
from whatsapp_automation.questions_card import questions      # change these two for card/peln flow
# from question_peln import questions     # change these two for card/peln flow
import sys
...
sys.stdout = Logger(r"C:/Users/Diyanah/Documents/Selenium Log/17022020-01.txt")
...
# Get text list
def get_text_list(title, note, *index):
...
def loop_send_replies(a):          # how many times want to run through latest question in dictionary and send replies
    try:
        for i in range(a):
            ...
            if str.__contains__(msg[-1], "our terms of use"):
            ...
            elif str.__contains__(msg[-1], "back of your IC"):
                attach_icon = driver.find_element_by_xpath('//div[@title = "Attach"]')
                attach_icon.click()
                image_icon = driver.find_element_by_xpath(
                    '//input[@accept = "image/*,video/mp4,video/3gpp,video/quicktime"]')
                ...
                get_text_list("Top 3 products are: ", "User choose the first product", -12, -9, -6)   # "2"

            elif str.__contains__(msg[-5], "branch"):
                get_text_list("Branches are: ", "User select the first branch", -8, -6, -4, -2)       # "1"
...
quick_send("test rhbk card", "Package1 rhb card flow")
print()
print()


# Start looping through the questions
loop_send_replies(55)
sleep(wait)
